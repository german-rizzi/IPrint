...
namespace IPrint.Repositories
{
	public class PrintSpoolerRepository
	{
		...
		public async Task<IEnumerable<PrintSpooler>> GetByStatusAsync(int status, int equipId, int companyId)
		{
			...
			string query = "SELECT Id, UrlArchivo as Url, Printed, Porait, PrinterId, IdEquipo as EquipId, IdEmpresa as CompanyId  FROM print_spooler WHERE idEmpresa = @CompanyId and EquipoId = @EquipId and printed = @Printed";
			return await connection.QueryAsync<PrintSpooler>(query, new { CompanyId = companyId, EquipId = equipId, Printed = status });
		}
